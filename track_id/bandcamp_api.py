...
class BandcampDataSource(DataSource):
    ...
    def enrich_mp3_file(self, file_path: str) -> Dict[str, Any]:
        ...
        bandcamp_metadata = self.extract_metadata(matching_track)
...
def search_bandcamp(search_text: str) -> Dict[str, Any]:
    """Legacy function - use BandcampDataSource.search() instead."""
    source = BandcampDataSource()
    return source.search(search_text)


def find_matching_track(search_results: Dict[str, Any], artist: str, title: str) -> Optional[Dict[str, Any]]:
    """Legacy function - use BandcampDataSource.find_matching_track() instead."""
    source = BandcampDataSource()
...
def extract_bandcamp_metadata(track_data: Dict[str, Any]) -> Dict[str, Any]:
    """Legacy function - use BandcampDataSource.extract_metadata() instead."""
    source = BandcampDataSource()
    return source.extract_metadata(track_data)


def enrich_mp3_file(file_path: str) -> Dict[str, Any]:
    """Legacy function - use BandcampDataSource.enrich_mp3_file() instead."""
    source = BandcampDataSource()
